...
CONF.register_opt(cfg.StrOpt('smtp_user', default=''), group='spassword')
CONF.register_opt(cfg.StrOpt('smtp_password', default=''), group='spassword')
CONF.register_opt(cfg.StrOpt('smtp_from', default=''), group='spassword')
...
class SPasswordManager(manager.Manager):
    ...
    def user_get_sndfa(self, user_id):
        LOG.info("User %s get sndfa in driver manager" % user_id)
        return self.driver.get_sndfa(user_id)
